...
const suggestionSchema = z.object({
  suggestion: z
    .string()
    .describe(
      "The code to insert at cursor, or empty string if no suggestion needed.",
    ),
});
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!code) {
      throw NextResponse.json({ error: "Code is required" }, { status: 400 });
    }

    const prompt = SUGGESTION_PROMPT.replace("{fileName}", fileName)
      .replace("{code}", code)
      .replace("{curentLine}", currentLine)
