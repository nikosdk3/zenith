  ...
  return [
    {
      ...
      create() {
        ...
        cancelButton.className =
          "font-sans p-1 px-2 text-muted-foreground hover:text-foreground hover:bg-foreground/10 rounded-sm transition-colors";
...
const quickEditKeymap = keymap.of([
  {
    ...
    run: (view) => {
      ...
      const isOpen = view.state.field(quickEditState);

      view.dispatch({
        effects: showQuickEditEffect.of(!isOpen),
      });
