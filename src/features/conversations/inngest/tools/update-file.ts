...
export const createUpdateFileTool = ({
  ...
  return createTool({
    ...
    handler: async (params, { step: toolStep }) => {
      ...
      // Validate file exists before trying to update it
      ...
      if (file.type === "folder") {
        return `Error: "${fileId}" is a folder. You can only update file contents.`;
      }
      ...
      try {
        return await toolStep?.run("update-file", async () => {
          ...
          return `File "${file.name}" was updated successfully.`;
        });
      } catch (error) {
